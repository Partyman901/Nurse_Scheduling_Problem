...
def calculationTotalAversion(prefs, assignment):
  ...
  for shift in assignment:
      for i in range(len(shift)):
          ...
          # print(f"MY PRINT@@@, {assignment}")
          # print(f"PREFS INPUT WEIRD THING", prefs_input[shift[i]-1][i])
          # print(f"PREFS INPUT IS {prefs_input}")
          # print(f"ITERARTION {i} \n")
          nurse_ans += prefs_input[shift[i]-1][i]
          # print("Regular prefs input", prefs_input)
          # print("CALCULATE PREFS INPUT", prefs_input[shift[i]-1][i])
  ans += nurse_ans
